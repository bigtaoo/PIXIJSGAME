...
export class Header extends BaseHeader {
  ...
  protected getTarget(): number { return this._target; }
  ...
  private updateWarnShake(deltaMs: number): void {
    const warn = this.lastDisplayedSeconds > 0 && this.lastDisplayedSeconds < BaseHeader.WARN_THRESHOLD;
    if (warn) {
  ...
  private updateHighlight(deltaMs: number): void {
    ...
    const targetTint = (this.lastDisplayedSeconds > 0 && this.lastDisplayedSeconds < BaseHeader.WARN_THRESHOLD)
      ? 0xFF5252 : 0xFFFFFF;

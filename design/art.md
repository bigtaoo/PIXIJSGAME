...
| 填色 | 顶部纯白 → 底部暖米色 `#F0E4C8` 渐变，顶部 25% 高度叠加白色高光（alpha 衰减），整体保持浅色可读性 |
| 对齐 | 视觉居中（非数学居中，需手动微调） |
...
| 层 | 内容 | 参数 |
|----|------|------|
| Layer 1：底部阴影条 | 同底色加深 28%，圆角矩形，偏移 `size × 10%` | 营造厚度感 |
| Layer 2：主体面 | 当前 tier 颜色，圆角矩形，高度 `size - depth` | 正面可见区域 |
| Layer 3：高光椭圆 | 白色 alpha 0.38，随机数量 / 位置 | 模拟光泽反射 |
...
| Tier | 颜色 | hex | 对应数值范围 |
|------|------|-----|------------|
| 0 — 小数 | 柔和天蓝 | `#C5E8FA` | `value ≤ (target-1) / 3` |
| 1 — 中数 | 暖奶油 | `#FFF3CC` | `value ≤ (target-1) × 2/3` |
| 2 — 大数 | 柔珊瑚 | `#FFCCBC` | 其余 |
